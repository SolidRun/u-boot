					...
					bool cmd_not_dat)
{
	...
	if (phy->settings.sdhc_extended_wr_mode == 0) {
		...
		if (n <= phy->vars.dll_max_value) {
			...
			printf("Warning Expected DLL value setting exceeds DLL capabilities. Applied maximum DLL setting.\n");
			clk_wr_delay = 255;
			/* no dll setting*/
		}
	} else {
...
static int sdhci_cdns_probe(struct udevice *dev)
{
	...
	if (ret)
		return ret;
	sdhci_cdns_sd6_stop_clock(plat);
#endif
