...
static int octeontx_tune_hs400(struct mmc *mmc)
{
	...
	if (env_get_yesno("emmc_export_hs400_taps") > 0) {
		...
		snprintf(env_name, sizeof(env_name),
			 ...
			 slot->bus_id);
		env_set_ulong(env_name, slot->cmd_out_taps_delay[MMC_HS_400]);
		snprintf(env_name, sizeof(env_name),
		...
		snprintf(env_name, sizeof(env_name),
			 ...
			 slot->bus_id);
		env_set_ulong(env_name, slot->data_out_taps_delay[MMC_HS_400]);
	} else {
				      ...
				      u32 opcode)
{
	...
	if (is_hs200 && env_get_yesno("emmc_export_hs200_taps") == 1) {
		...
		default:
			...
			env_set_ulong(env_name, slot->data_out_taps_delay[MMC_HS_200]);
			snprintf(env_name, sizeof(env_name),
				 "emmc%d_hs200_cmd_out_tap", slot->bus_id);
			env_set_ulong(env_name, slot->cmd_out_taps_delay[MMC_HS_200]);
			break;
		}
	}
...
static int octeontx_mmc_get_config(struct udevice *dev)
{
	...
	slot->cfg.f_max = ofnode_read_s32_default(dev_ofnode(dev), "max-frequency",
						  26000000);
	...
	err = ofnode_read_u32_array(dev_ofnode(dev), "voltage-ranges", voltages, 2);
	if (err) {
