...
static void *nix_memalloc(int num_elements, size_t elem_size, const char *msg)
{
	...
	void *base = memalign(NIX_CACHELINE_SIZE, alloc_size);
...
{
	int index;
	u64 lmt_data[2], val, lmt_addr = readq(npa->lmt_base + (npa->lf * 0x1000));

	for (index = 0; index < queue_length; index++) {
		buffers[index] = memalign(NIX_CACHELINE_SIZE,
					  buffer_size);
		if (!buffers[index]) {
...
int npa_lf_setup(struct nix *nix, int npa_lf)
{
	...
	npa->lmt_base = (void *)readq(CAVM_APR_AF_LMT_MAP_BASE);
	...
	for (idx = 0; idx < NPA_POOL_COUNT; idx++) {
		...
		debug("%s aura %p pool %p[%d]\n", __func__, aura, pool, idx);
		...
		pool->s.buf_size =
			npa->buf_size[idx] / NIX_CACHELINE_SIZE;
...
int npa_lf_shutdown(struct nix *nix)
{
	...
	err = npa_lf_admin_shutdown(nix->nix_af->npa_af, npa->lf,
				    NPA_POOL_COUNT);
	if (err) {
...
struct nix *nix_lf_alloc(struct udevice *dev, int nix_id)
{
	...
	nix->lmt_base = (void *)readq(CAVM_APR_AF_LMT_MAP_BASE);
				 ...
				 int num_words)
{
	...
	u64 *lmt_ptr = (u64 *)(readq(nix->lmt_base + (nix->npa->lf * 0x1000)) + (0x80 * 0x10));
	u64 *ptr = buffer;
