		    ...
		    const union npa_pool_s *desc, u32 pool_id)
{
	...
	context = (union npa_pool_s *)(npa->aq.res.base +
						NIX_CACHELINE_SIZE);
	memset(npa->aq.res.base, 0, npa->aq.res.entry_sz);
	memcpy(context, desc, sizeof(union npa_pool_s));
	__iowmb();
	npa_af_reg_write(npa, NPA_AF_AQ_DOOR(), 1);
...
int npa_lf_admin_setup(struct npa *npa, int lf, dma_addr_t aura_base)
{
	...
	auras_cfg.s.way_mask = 0xffff; //Bypass NDC
...
int npa_set_pool_stackpages (struct npa *npa, int lf, int pool_id)
{
	u32 head;
	ulong start;
	union npa_aq_inst_s *inst;
	...
	aq_stat.u = npa_af_reg_read(npa->npa_af, NPA_AF_AQ_STATUS());
	head = aq_stat.s.head_ptr;
	inst = (union npa_aq_inst_s *)(npa->npa_af->aq.inst.base) + head;
	res = &pool_req.resp;
	memset(inst, 0, sizeof(*inst));
	inst->s.cindex = pool_id;
	inst->s.lf = lf;
	...
	pool_req.p0.s.stack_pages = npa->stack_pages[pool_id];
	pool_req.p1.s.stack_pages = -1;	/* Write mask */
	__iowmb();
	npa_af_reg_write(npa->npa_af, NPA_AF_AQ_DOOR(), 1);
	start = get_timer(0);
	while ((res->s.compcode == NPA_AQ_COMP_E_NOTDONE) &&
	       (get_timer(start) < 1000))
		schedule();
...
int npa_af_setup(struct npa_af *npa_af)
{
	...
	/* Enable NDC cache */
	//TODO:ndc_cfg.u = npa_af_reg_read(npa_af, NPA_AF_NDC_CFG());
	//TODO:ndc_cfg.s.ndc_bypass = 1;
	//TODO:npa_af_reg_write(npa_af, NPA_AF_NDC_CFG(), ndc_cfg.u);
	/* Set up queue size */
				...
				int cindex, union nix_aq_res_s *resp)
{
	...
	do {
		schedule();
		dsb();
	} while (result->s.compcode == 0 && get_timer(start) < 20);
...
int npc_lf_admin_setup(struct nix *nix)
{
	...
	kex_cfg.u = 0;
	kex_cfg.s.keyw = NPC_MCAMKEYW_E_X2;
	...
	camx_intf.u = 0;
	camx_intf.s.keyw = ~NPC_MCAMKEYW_E_X2;
	...
	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W2_EXT(pkind, 0, 0),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W2_EXT(pkind, 0, 1),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W3_EXT(pkind, 0, 0),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W3_EXT(pkind, 0, 1),
			 0);
	...
	camx_intf.s.intf = ~NPC_INTF_E_NIXX_RX(nix_id);
	npc_af_reg_write(nix_af,
			 NPC_AF_MCAMEX_BANKX_CAMX_INTF_EXT(pkind, 1, 0),
			 camx_intf.u);
	...
	camx_intf.s.intf = NPC_INTF_E_NIXX_RX(nix_id);
	npc_af_reg_write(nix_af,
			 NPC_AF_MCAMEX_BANKX_CAMX_INTF_EXT(pkind, 1, 1),
			 camx_intf.u);

	camx_w0.u = 0;
	camx_w0.s.md = ~(nix->lmac->chan_num) & (~((~0x0ull) << 12));
	npc_af_reg_write(nix_af,
			 NPC_AF_MCAMEX_BANKX_CAMX_W0_EXT(pkind, 1, 0),
			 camx_w0.u);

	camx_w0.u = 0;
	camx_w0.s.md = nix->lmac->chan_num;
	npc_af_reg_write(nix_af,
			 NPC_AF_MCAMEX_BANKX_CAMX_W0_EXT(pkind, 1, 1),
			 camx_w0.u);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W1_EXT(pkind, 1, 0),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W1_EXT(pkind, 1, 1),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W2_EXT(pkind, 1, 0),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W2_EXT(pkind, 1, 1),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W3_EXT(pkind, 1, 0),
			 0);

	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_CAMX_W3_EXT(pkind, 1, 1),
			 0);
	...
	npc_af_reg_write(nix_af,
			 //TODO:NPC_AF_MCAMEX_BANKX_STAT_ACT_EXT(pkind, 0),
			 NPC_AF_MCAMEX_BANKX_CFG_EXT(pkind, 0),
			 mcamex_stat_act.u);

	intfx_stat_act.u = 0;
	intfx_stat_act.s.ena = 1;
	intfx_stat_act.s.stat_sel = 16;
	offset = CAVM_NPC_AF_INTFX_MISS_STAT_ACT(NPC_INTF_E_NIXX_RX(nix_id));
	npc_af_reg_write(nix_af, offset, intfx_stat_act.u);
#endif
	rx_action.u = 0;
	rx_action.s.pf_func = nix->pf_func;
	rx_action.s.op = NIX_RX_ACTIONOP_E_UCAST;
	npc_af_reg_write(nix_af, NPC_AF_MCAMEX_BANKX_ACTIONX_EXT(pkind, 0, 0),
			 rx_action.u);
	...
	npc_af_reg_write(nix_af,
			 NPC_AF_INTFX_MISS_ACT(NPC_INTF_E_NIXX_RX(nix_id), 0),
			 rx_action.u);
	bankx_cfg.u = 0;
	bankx_cfg.s.ena = 1;
	...
	npc_af_reg_write(nix_af,
			 NPC_AF_INTFX_MISS_ACT(NPC_INTF_E_NIXX_TX(nix_id), 0),
			 tx_action.u);
