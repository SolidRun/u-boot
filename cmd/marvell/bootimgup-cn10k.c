...
/**
 * cn10k_kernel_bootm() - boot cn10k kernel directly
 *
 */
static int cn10k_kernel_bootm(void *start_addr, char *loadaddr, char *fdtaddr, int length)
{
	static char load_addr[32];
	static char fdt_addr[32];
	static char *const bootm_args[] = {
		"bootm", load_addr, load_addr, fdt_addr, NULL
	};

	snprintf(load_addr, sizeof(load_addr) - 1,
		 "%s", loadaddr);
	snprintf(fdt_addr, sizeof(fdt_addr) - 1,
		 "%s", fdtaddr);

	printf("Booting kernel at %s ...\n\n\n", load_addr);

	do_bootm(NULL, 0, 4, bootm_args);
	//do_booti(NULL, 0, 3, bootm_args);
 ...
 *
 */
static int cn10k_kernel_booti(void *start_addr, char *loadaddr, char *fdtaddr, int length)
{
	static char load_addr[32];
	static char fdt_addr[32];
	static char *const bootm_args[] = {
		"bootm", load_addr, "-", fdt_addr, NULL
	};
...
static int cn10k_test_mmc_read(void *addr, u32 blk, u32 cnt)
{
	struct mmc *mmc;
	...
	n = blk_dread(desc, blk, cnt, addr);
	if (n != cnt) {
		printf("ERROR: read partition table failed\n");
		return CMD_RET_FAILURE;
	}

	printf("%s blocks read %d\n", __func__, n);
}

static int cn10k_boot_mmc_read(void *addr, u32 blk, u32 cnt)
{
	int n;
	void *key_addr = NULL;
	struct mmc *mmc;
	...
	void *relocated = (void *)map_sysmem(addr, 0);

	n = blk_dread(mmc_get_blk_desc(mmc), blk, cnt, relocated);
	printf("%d blocks read: %s\n", n, (n == cnt) ? "OK" : "ERROR");

	printf("%d RPMB blocks read: %s\n", n, (n == cnt) ? "OK" : "ERROR");
	if (n != cnt) {
		printf("%d FAILURE RPMB blocks read: %s\n", n, (n == cnt) ? "OK" : "ERROR");
		return CMD_RET_FAILURE;
	}
	return CMD_RET_SUCCESS;
}
...
static int do_boot_application_image(struct cmd_tbl *cmdtp, int flag, int argc,
				     char * const argv[])
{
	struct smc_version_info vinfo;
	unsigned long value;
	unsigned long loadaddr;
	unsigned long fdtcontroladdr;
	int ret;
	bool mmc = false;
	bool spi = false;
	bool verify = false;
	...
	//cn10k_test_mmc_read((void *)0x20080000, 0x2001000/0x200, 0x1a61200/0x200 );
	while (argc > 0) {
		pr_debug("Parsing argument \"%s\"\n", argv[0]);
		if (!strcmp(argv[0], "-v")) {
			verify = true;
	...
	mmc_dev = find_mmc_device(curr_device);
	...
	for (i = 0; i < vinfo.num_objects; i++) {
		...
		if (!strcmp(object->name, "Image")) {
			...
			ret = strict_strtoul(load_addr, 16, &loadaddr);
			ret = strict_strtoul(fdt_addr, 16, &fdtcontroladdr);

			cn10k_boot_mmc_read((void *)loadaddr, object->object_address / 512,
					    object->object_size / 512 + 1);
			printf("Calling cn10k_kernel_boot\n");
			cn10k_kernel_bootm((void *)loadaddr, load_addr, fdt_addr,
					   object->object_size);
		}
	}
	return CMD_RET_SUCCESS;
...
U_BOOT_CMD(mmcbootm, 5, 1, do_boot_application_image,
	   "Boot application image",
	   " <mmc [devid]>\n")

static int do_boot_kernel_image(struct cmd_tbl *cmdtp, int flag, int argc,
				char * const argv[])
{
	struct smc_version_info vinfo;
	unsigned long value;
	...
	for (i = 0; i < vinfo.num_objects; i++) {
		...
		if (!strcmp(object->name, "Image")) {
			...
			cn10k_boot_mmc_read((void *)loadaddr, object->object_address / 512,
					    object->object_size / 512 + 1);
			printf("Calling cn10k_kernel_booti\n");
			cn10k_kernel_booti((void *)loadaddr, load_addr, fdt_addr,
					   object->object_size);
		}
	}
	return CMD_RET_SUCCESS;
...
U_BOOT_CMD(mmcbooti, 5, 1, do_boot_kernel_image,
	   "Boot kernel image",
	   " <mmc [devid]>\n")
#endif // EMMC_NS_READONLY
